...
|---|---|---|---|
| `Finding 1` | Default credentials on the PLC web management interface | **Remediated (Aug 27, 2026)** — consistent across every reference, no remaining contradiction (see inconsistency #1) | `Artifact-3-Segmentation-Assessment.md`:23, 62, 92, 207<br>`AWWA-Assessment-Shenandoah-Valley-Water-Authority.md`:36, 65, 114<br>`Crosswalk-NIST80053-IEC62443-Shenandoah-Valley.md`:18, 35<br>`EPA-Checklist-Shenandoah-Valley-Water-Authority.md`:31, 96<br>`ERP-Shenandoah-Valley-Water-Authority.md`:60, 78<br>`RRA-Shenandoah-Valley-Water-Authority.md`:7*, 50, 91, 92, 101 |
| `Finding 2` | Historian database access controlled at the application layer only | `Remediated (Aug 26, 2026)` — network-layer segmentation added and re-verified Aug 31, 2026 (`(High)` at time of discovery, Artifact-3:101 · RRA:51) | `Artifact-3-Segmentation-Assessment.md`:101<br>`RRA-Shenandoah-Valley-Water-Authority.md`:7*, 51 |
| `Finding 3` | Lab-build defect: Configuration-vs-enforcement gap on PLC and historian firewalls | `Remediated (Aug 26, 2026)` — `firewall=1` set on both hosts, live enforcement confirmed and re-verified Aug 31, 2026 (`(Medium, methodological)` at time of discovery, Artifact-3:104 · RRA:52) | `Artifact-3-Segmentation-Assessment.md`:104, 191<br>`AWWA-Assessment-Shenandoah-Valley-Water-Authority.md`:36, 48, 52<br>`Crosswalk-NIST80053-IEC62443-Shenandoah-Valley.md`:21<br>`EPA-Checklist-Shenandoah-Valley-Water-Authority.md`:45<br>`RRA-Shenandoah-Valley-Water-Authority.md`:7*, 52, 104<br>`Artifact-6-Threat-Detection-Assessment.md`:65 (cited by name, not status-bearing) |
| `Finding 4` | HMI web interface unencrypted | `(Medium)` (Artifact-3:107) · `**Medium**` (RRA:53) · `remains open` (RRA:93) | `Artifact-3-Segmentation-Assessment.md`:107<br>`Crosswalk-NIST80053-IEC62443-Shenandoah-Valley.md`:22<br>`EPA-Checklist-Shenandoah-Valley-Water-Authority.md`:41<br>`RRA-Shenandoah-Valley-Water-Authority.md`:7*, 53, 93, 103 |
| `Finding 3.1` | Lab-build defect: PLC-to-historian egress anomaly — root cause identified but unresolved (Artifact 3, Appendix A) | `**Low / Informational**` (RRA:54) · root cause diagnosed via toggle test on VM104; symptom cleared by host reboot ~Aug 28–30, 2026; underlying condition persists, not permanently remediated (Artifact-3:234) | `RRA-Shenandoah-Valley-Water-Authority.md`:54, 69, 105<br>`Artifact-3-Segmentation-Assessment.md`:213, 234, 238 |
| `Finding 6.1` | Lab-build defect: Stock `suricata.yaml` referenced a nonexistent capture interface (`eth0` vs. actual `ens18`), causing a 14,000+ restart crash loop while reporting `active` | `Remediated (2026)` — interface corrected, zero restarts confirmed over a multi-day period (source doc uses narrative "Remediated," no discrete date given) | `Artifact-6-Threat-Detection-Assessment.md`:53, 65 |
| `Finding 6.2` | Custom detection rule silently not loaded due to a `default-rule-path` resolution mismatch (`/etc/suricata/rules/` vs. actual `/var/lib/suricata/rules`) | `Remediated (2026)` — rule relocated, loaded-rule count confirmed incremented (source doc uses narrative "Remediated," no discrete date given) | `Artifact-6-Threat-Detection-Assessment.md`:61 |
| `Finding 6.3` | Sensor placement behind a default-deny firewall structurally limits visibility to permitted traffic only | `Open` — High significance; source doc's posture table marks it `❌ Not achieved`; not a remediation-track item, it is a structural limitation of the current architecture | `Artifact-6-Threat-Detection-Assessment.md`:98, 171, 196, 208, 238 |
| `Finding 6.4` | Two-path detection evaluated (Sept 17, 2026 retest): wire-level (Suricata, stock ET Open) produces zero alerts on SSH credential-guessing despite the sensor demonstrably inspecting the traffic live (positive control verified in-window); host-level (sshd auth.log) captured all 8 test attempts with correct source attribution, serving as a verified compensating control | `Open (wire-level) — host-level compensating control verified` — wire-level gap remains open (no stock rate-based detection signature exists in ET Open); host-level compensating control demonstrated working (Sept 17, 2026 retest: 8/8 credential-guessing attempts captured in `auth.log` with correct source attribution). High significance; source doc's Section 6 posture table still marks wire-level credential-attack detection `❌ Not achieved`. | `Artifact-6-Threat-Detection-Assessment.md`:120, 172, 206 |
...
RRA — `Finding (ref. Artifact 3)`, RRA:48), `Artifact-6-Threat-Detection-Assessment.md` for
