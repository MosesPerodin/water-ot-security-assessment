...
| **AC-4** (Information Flow Enforcement) | FR5 — Restricted Data Flow; explicit conduit authorization | **Partially implemented.** Two cross-zone conduits are authorized and enforced (HMI↔PLC Modbus/502, HMI↔Historian PostgreSQL/5432); administrative SSH access (192.168.30.0/24 to all OT zones) is also documented in policy but represents a wider authorization than the two operational conduits | Artifact 3, Section 5 (Segmentation Design); conduit connectivity tests and firewall policy inspection |
...
| **IA-5** (Authenticator Management) | FR1 — Identification and Authentication Control | **Partially implemented.** Default vendor credentials on the PLC management interface were identified and replaced (Aug 27, 2026); old credentials verified dead, new credentials verified working. Password policy, rotation cadence, complexity requirements, and multi-factor authentication remain unaddressed | Artifact 3, Finding 1 (remediated); RRA Section 3/4; EPA Checklist 2.A (now ✅); FINDINGS-REGISTER Finding 7.1 (no MFA) |
