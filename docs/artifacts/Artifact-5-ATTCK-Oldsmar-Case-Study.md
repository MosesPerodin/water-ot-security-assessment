...
|---|---|---|
| SCADA workstation directly internet-connected | A NAT egress path (MASQUERADE) exists for three OT subnets — Monitoring, Field/Control, and Supervisory (LAB-TOPOLOGY.md, NAT table); administrative inter-zone SSH access is also permitted and documented (Artifact 3) | **Materially stronger, not absolute.** The direct entry vector Oldsmar had — a SCADA host reachable inbound from the internet — doesn't exist here, but outbound NAT egress from three OT subnets makes the internet-exposure vector reduced and indirect rather than fully absent; administrative SSH pathways between zones also remain authorized |
