...
**Note on prior (August 19) evidence:** an earlier, separately-stored evidence set from Phase 1/2 predates service installation and the segmentation work itself, and is not used as evidence in this document. The August 25/26 set provides the before-baseline; the HMI was remediated on August 26 as part of the conduit-establishment work, and post-remediation testing resumed on August 26 with both HMI and firewall changes in place. See Section 1 and Appendix A for the remediation timeline.
